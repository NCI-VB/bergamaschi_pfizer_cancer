---
title: "DEG and Volcano Plots (Low Only)"
...
```{r deg, echo=TRUE}
  ...
  targetfile$patient <- apply(array(targetfile$sample_id), 1, function(z) unlist(str_split(z,"_"))[1])
  
  low_samples <- c("C16", "C18", "C25", "C26")

  targetfile <- targetfile %>% filter(patient %in% low_samples)
  
  samples_for_deg_analysis = targetfile$sample_id #include everyone

  df.m <- df[,samples_for_deg_analysis]
  
  filter <- apply(df.m, 1, function(z) any(is.na(z)))
  df.m <- df.m[!filter,]
  
  gene_names <- NULL
  gene_names$GeneID <- rownames(df.m)
  genenames <- rownames(df.m)
  ...
  ordered_covariates=c("timepoint","patient")
  ...
  write.table(deg_results, file=file.path(workDir,"output","deg_analysis_only_low.tsv"), sep="\t", row.names = FALSE)
